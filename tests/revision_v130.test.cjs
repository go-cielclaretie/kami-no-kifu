...
for(const [dir,layouts] of [['Horizontal',['balanced','table','cards','bands']],['Vertical',['balanced','table','tiles','focus']]])test(`v130 adopted ${dir} layouts`,()=>{for(const x of layouts)assert.equal(K.validateSettings({...K.defaults(),['infoLayout'+dir]:x}).length,0);assert.ok(K.validateSettings({...K.defaults(),['infoLayout'+dir]:'groups'}).length)});
test('v130 local storage preference roundtrip',()=>{const map=new Map(),p=preferences({getItem:k=>map.get(k)||null,setItem:(k,v)=>map.set(k,v)});const input={...K.defaults(),grain:'itame',mode:'split',split:'50',infoLayoutVertical:'tiles'};p.save(input);assert.equal(p.load().settings.grain,'itame');assert.equal(p.load().settings.infoLayoutVertical,'tiles');assert.equal(p.load().error,'')});
