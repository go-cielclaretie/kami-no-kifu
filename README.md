...
- 横書き4種／縦書き4種の対局情報、6種類の持ち時間、所感の掲載ページ・ページ別本文。
...
フォント本体は含めていません。数字表記は追加設定なしで動作します。
漢数字では端末に入っている行書体を確認し、なければ手元のフォントファイルを選択できます。
運営者が適切な公開権限を持つWebフォントを同じサイトへ配置すれば、訪問者向けの自動読み込みにも対応します。
...
表示されたlocalhost URLをブラウザで開きます。HTTP確認サーバーは自分のPCからだけ接続できる設定です。
`index.html`をファイルとして直接開くと、ブラウザの制約で画像・フォント・保存が制限される場合があります。
手早く画面だけ試す場合は `standalone/index.html` も使えますが、設定保存の確認はHTTP/HTTPSで行ってください。
